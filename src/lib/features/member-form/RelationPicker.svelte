...
<div class="space-y-4 text-left">
	<!-- Pilihan Ayah -->
	<div>
		<label for="rel-father-select" class="block text-xs font-semibold text-[var(--color-text-secondary)] mb-1">
	...
	<!-- Pilihan Ibu -->
	<div>
		<label for="rel-mother-select" class="block text-xs font-semibold text-[var(--color-text-secondary)] mb-1">
	...
	<div>
		...
		<div class="flex flex-wrap gap-2 max-h-36 overflow-y-auto p-2 border border-[var(--color-border)] rounded-[var(--radius-md)] bg-[var(--color-surface-muted)]">
			{#if spouseCandidates.length === 0}
				<span class="text-xs text-[var(--color-text-secondary)] italic">Tidak ada kandidat pasangan yang cocok</span>
			{:else}
				{#each spouseCandidates as candidate}
					{@const isSelected = spouseIds.includes(candidate.id)}
					<button
						type="button"
						class="px-2.5 py-1 text-xs font-semibold rounded-[var(--radius-sm)] border transition-colors {isSelected
	...
	<div>
		...
		<div class="flex flex-wrap gap-2 max-h-36 overflow-y-auto p-2 border border-[var(--color-border)] rounded-[var(--radius-md)] bg-[var(--color-surface-muted)]">
			{#if childrenCandidates.length === 0}
				<span class="text-xs text-[var(--color-text-secondary)] italic">Tidak ada kandidat anak</span>
			{:else}
